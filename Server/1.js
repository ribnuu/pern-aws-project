...
app.use((req, res, next) => {
  ...
  // Handle case insensitivity for 'user_id' header
  const userId =
    req.headers["user_id"] || req.headers["User-Id"] || req.headers["user-id"];

  if (!userId) {
    console.error("USER ID IS NOT AVAILABLE FOR THIS REQUEST");
    return res.status(400).json({ error: "Malicious request detected" });
  } else {
    console.log("USER ID IS AVAILABLE FOR THIS REQUEST: ", userId); // Debug the userId value
  }
