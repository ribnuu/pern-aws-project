...
// ! IMPORTANT
// CREATE SEQUENCE IF NOT EXISTS public.license_reference_number_sequence
//     INCREMENT 1
//     START 0
//     MINVALUE 0
...
const getNextSequence = async (sequilize) => {
  const result = await sequilize.query(
    "SELECT nextval('license_reference_number_sequence')"
  );
  const sequenceNumber = result[0][0]["nextval"];
...
const generateTrafficFineReferenceNumber = async () => {
  try {
    const { sequelize } = setUpAssociationsCCC();
    // let count = 0;
    // while (count <= 9999) {
    ...
    const dayOfYear = getDayOfYear();
    const alpCode = getAlphabeticCode(dayOfYear);
    const sequenceCode = await getNextSequence(sequelize);
