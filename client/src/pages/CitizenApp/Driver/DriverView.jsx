...
const DriverView = () => {
  ...
  const driverByNICNumber = async (nic_number) => {
    try {
      const driverByNICNumberResponse = await axios.post(
        `http://${server_port}:4000/api/driver/ViewDriver`,
        {
          nic_number,
        }
      );

      setDriverData(driverByNICNumberResponse.data.rows);
      console.log(driverByNICNumberResponse.data.rows);
    } catch (error) {
      console.error(error);
    }
  };
