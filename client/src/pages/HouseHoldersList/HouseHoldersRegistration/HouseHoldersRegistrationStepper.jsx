...
// const HouseHoldersRegistrationStepper = ({ steps, currentStep }) => {
//   return (
...
// export default HouseHoldersRegistrationStepper;

// import React from "react";

// // <ol
// //   className="flex items-center w-full font-bold text-center  "
...
//             } sm:after:content-[''] after:w-full after:h-2 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-12 xl:after:mx-20 dark:after:border-gray-700`}
//           >
//             {/* <span className="flex items-center after:content-['/'] sm:after:hidden after:mx-4 after:text-gray-200 dark:after:text-gray-500 cursor-pointer"> */}
//             <span className="flex items-center after:content-[''] sm:after:hidden after:mx-4 after:text-gray-200 dark:after:text-gray-500 cursor-pointer">
//               {/* Show completed icon if step is completed */}
//               {isCompleted
//                 ? step.iconCompleted || (
//                     <span className="me-4">{stepNumber}</span>
//                   )
//                 : /* Show pending or current icon depending on the step's state */
//                   step.iconPending || (
//                     <span className="me-4">{stepNumber}</span>
//                   )}
//               <span
//                 className="hidden sm:inline-flex sm:ms-4"
//                 style={{ fontWeight: "bolder" }}
//               >
//                 {step.title}
//               </span>
...
const HouseHoldersRegistrationStepper = ({ steps, currentStep }) => {
  return (
    <div className="w-full">
      {/* Stepper Row (Desktop view remains the same) */}
      <ol className="flex items-center w-full font-medium text-center">
        {steps.map((step, index) => {
          const stepNumber = index + 1;
          const isCompleted = currentStep > stepNumber; // Completed steps
          ...
          return (
            ...
            >
              <span className="flex items-center after:content-[''] sm:after:hidden after:mx-2 after:text-gray-200 dark:after:text-gray-500 cursor-pointer">
                ...
                {/* Title visible for larger screens */}
                <span
      ...
      {/* Mobile View - Show only the current step's title below the stepper */}
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div
              key={index}
              className={`${
                isActive ? "block text-lg font-semibold" : "hidden"
              }`}
            >
              {step.title}
            </div>
