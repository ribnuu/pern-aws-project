...
import SelectionGroupsNewSection from "./SelectionGroupsNewSection";
import "./styles.css"; // Import your custom styles
import {
  getRightsTreeViewData,
  updateRightsTreeViewData,
} from "../../../apis/CccRightsApiService";
import SelectionRolesNewSection from "./SelectionRolesNEwSection";
...
const AssignRightsTreeView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (selectedOption == "Groups New" && selectedGroupNew) {
        await getTreeViewDataForUserButtonRightsByGroupNew();
      }
    };
    fetchData();
    return () => {};
  }, [selectedOption, selectedGroupNew]);

  useEffect(() => {
    const fetchData = async () => {
      if (selectedOption == "Roles New" && selectedRoleNew) {
        await getTreeViewDataForUserButtonRightsByRoleNew();
      }
    };
    fetchData();
    return () => {};
  }, [selectedOption, selectedRoleNew]);
  ...
  const getTreeViewDataForUserButtonRightsByGroupNew = async () => {
    try {
      const response = await getRightsTreeViewData({
        groupId: selectedGroupNew,
        roleId: null,
      });
  ...
  const getTreeViewDataForUserButtonRightsByRoleNew = async () => {
    try {
      const response = await getRightsTreeViewData({
        groupId: null,
        roleId: selectedRoleNew,
      });
      if (response && response.success) {
        setComponentData(response.data);
      }
  ...
  const updateGroupsNewRightsToDatabase = async (user_group_id, nodes) => {
    try {
      const response = await updateRightsTreeViewData({
  ...
  const updateRolesNewRightsToDatabase = async (user_role_id, nodes) => {
    try {
      const response = await updateRightsTreeViewData({
        roleId: user_role_id,
        data: nodes,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (selectedOption === "Groups New") {
        await updateGroupsNewRightsToDatabase(selectedGroupNew, nodes);
      } else if (selectedOption === "Roles New") {
        await updateRolesNewRightsToDatabase(selectedRoleNew, nodes);
      }
    } catch (error) {
      console.error(error);
    }
    ...
    <div className="mx-5 mt-8 bg-gray-800 p-4 rounded-lg py-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-lg shadow dark:border md:mt-0 w-full xl:p-0 dark:bg-gray-800 dark:border-white">
          ...
          {/* Group New */}
          {selectedOption && selectedOption === "Groups New" && (
            <SelectionGroupsNewSection />
          )}

          {selectedOption && selectedOption === "Roles New" && (
            <SelectionRolesNewSection />
          )}
        </div>
