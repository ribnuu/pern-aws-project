...
const UMSAssignRightsBackup = () => {
  const dbData = [
    ...
    },
    {
      ...
      children: [
        {
          label: "ASSIGN POLICE STATION - User - Duplicate",
          id: "1005000000000004",
        },
        {
          label: "Child 2",
  ...
  const getSubComponents = async (e) => {
    try {
      const subComponentsReponse = await getSubComponentsApi();
      if (subComponentsReponse.rowCount > 0) {
        setSubComponentList(subComponentsReponse.rows);
      }
    } catch (error) {
      console.error(error);
    }
    ...
    try {
      ...
      componentResponse.rows.forEach((componentElement) => {
        ...
        buttonResponse.rows.forEach((buttonElement) => {
          let component_pages_id = buttonElement.component_pages_id;
          let button_id = parseInt(buttonElement.ccc_master_buttons_id);

          if (pages_id === component_pages_id) {
            buttonChild.push({
              label: buttonElement.button_display_name,
              id: button_id,
            });
          }
        });
        let addComponent = {
          label: componentElement.component_name,
          id: componentElement.pages_id,
          children: buttonChild,
        };
        ComponentData.push(addComponent);
      });
      if (componentResponse.rowCount > 0) {
        setComponentList(componentResponse.rows);
      }
    } catch (error) {
      console.error(error);
    }
        ...
        <div className="rounded-lg shadow dark:border md:mt-0 w-full xl:p-0 dark:bg-gray-800 dark:border-white col-span-2">
          <div className=" space-y-4 md:space-y-6 sm:py-7 sm:px-1">
            <div className="h-auto">
              <DropdownTreeSelect
                showDropdown={"initail"}
                data={dbData}
                onChange={onChange}
                // className="bootstrap-demo w-96"
...
export default UMSAssignRightsBackup;
