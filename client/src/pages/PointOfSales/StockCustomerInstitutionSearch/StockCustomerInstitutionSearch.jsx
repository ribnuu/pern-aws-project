...
import { searchStockCustomerInstitutionApi } from "../../../apis/POSStockCustomerInstitutionApiService";

const StockCustomerInstitutionSearch = ({
  onSelectStockCustomerInstitution,
  setLoading,
  loading,
  searchTerm,
  setSearchTerm,
  ...
  const handleStockCustomerInstitutionSearch = async (event) => {
  ...
  const debouncedSearch = React.useCallback(
    _.debounce(async (searchTerm) => {
      setLoading(true);
      try {
        const response = await searchStockCustomerInstitutionApi(
          searchTerm,
          loadOnMount,
          loadRepresentative,
          loadAddress
  ...
  const handleSelectStockCustomerInstitution = (stockCusInst) => {
    ...
    onSelectStockCustomerInstitution(stockCusInst);
  };
  ...
  return (
    <div ref={containerRef} className="relative flex-1 mb-[8px]">
      {!hideLabel && (
        <label
          htmlFor="itemName2"
      ...
      <input
        ...
        className="w-full text-sm min-w-[260px] px-4 py-2  border border-gray-300 rounded-lg"
        placeholder="Search customer institution"
        onChange={handleStockCustomerInstitutionSearch}
      ...
      {searchResults.length > 0 && isClickedOnInput && !loading && (
        <div
          ...
          className="absolute z-10 bg-white border border-gray-300 rounded-md shadow-lg mt-1 min-w-full max-h-60 overflow-auto"
        >
...
export default StockCustomerInstitutionSearch;
