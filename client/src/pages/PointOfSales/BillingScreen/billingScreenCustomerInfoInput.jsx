...
const BillingScreenCustomerInfoInput = () => {
  ...
  const fetchStockBill = async (billNumber) => {
    if (!billNumber.trim()) {
      console.warn("Empty bill number, request aborted.");
      return;
    }

    console.log("Fetching bill details for:", billNumber);

    try {
      dispatch(fetchBillDetailsAsync(billNumber));
    } catch (error) {
            ...
            <div className=" flex items-center justify-center ">
              <label className="flex flex-col md:flex-row items-start mr-4 gap-2">
                ...
                <div className="flex items-center w-[35%] min-w-[160px] bg-gray-50 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600">
                  ...
                  />
                  <button
                    onClick={() => fetchStockBill(billNumber)}
...
export default BillingScreenCustomerInfoInput;
