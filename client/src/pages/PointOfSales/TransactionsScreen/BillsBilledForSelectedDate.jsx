...
import {
  ...
  // selectTotalQuantityFromBillPaymentSlice,
...
const BillsBilledForSelectedDate = ({ selectedStockCustomerInstitution }) => {
  const dispatch = useDispatch();
  const { billsBilledToSelectedDateRange, filters } = useSelector(
    ...
    const fetchData = async () => {
      try {
        const response = await getAllTransactionsByDateAndOtherFiltersApi({
          filterDateIn: "sbh",
          loadBillsBy: "All", // All, Pending, Paid, FullAndPartiallyPaid
        ...
        if (response.success) {
          dispatch(
            SetBillTransactionsSliceField({
              field: "billsBilledToSelectedDateRange",
              value: response.data,
            })
          );
        }
  ...
  useEffect(() => {
    if (
      ...
      billsBilledToSelectedDateRange.length > 0
    ) {
      const initialAmounts = billsBilledToSelectedDateRange.reduce(
        (acc, bill) => {
  ...
  return (
    <div className="">
      <h1 className="mb-2">Bills billed within the selected date range</h1>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {billsBilledToSelectedDateRange &&
              billsBilledToSelectedDateRange.length > 0 &&
              billsBilledToSelectedDateRange.map((pendingBill) => (
                <tr
            ...
            <tr className="bg-blue-100">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                {billsBilledToSelectedDateRange?.length} Bills
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
...
export default BillsBilledForSelectedDate;
