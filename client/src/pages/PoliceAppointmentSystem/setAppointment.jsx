...
const setAppointment = () => {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      const insertResponse = axios.post(
        "http://localhost:4000/policeAppointment/set",
        {
          formData,
        }
      );
  ...
  const fetchData = async () => {
    const response = await axios.get(
      "http://localhost:4000/policeAppointment/getAll"
    );
  ...
  return (
    <section className="mx-12 my-12">
      <div className="bg-white rounded-lg p-4 my-4">
        <h1 className="uppercase text-xl text-black mx-2 my-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="relative z-0 lg:w-full mb-6 groupflex justify-between gap-12">
            ...
            >
              <BiAddToQueue className="my-auto" />
              Reserve Slot
            </button>
          </div>
        </form>
      ...
      <div className="bg-white rounded-lg p-4 my-4">
        ...
        <div className="grid grid-cols-1  gap-4 my-4">
          {data.map((data) => {
            ...
            return (
              <div
                className="bg-gray-200 p-4 text-black text-sm w-102 rounded-lg space-between gap-6"
                key={data.police_appointment_id}
              >
                <div className="">
